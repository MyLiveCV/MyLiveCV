...
export class AuthService {
  ...
  async forgotPassword(email: string) {
    ...
    const subject = "Reset your My Live CV password";
  ...
  getAuthProviders() {
    ...
    if (
      this.configService.get("LINKEDIN_CLIENT_ID") &&
      this.configService.get("LINKEDIN_CLIENT_SECRET") &&
  ...
  async setup2FASecret(email: string) {
    ...
    const uri = authenticator.keyuri(email, "My Live CV", secret);
